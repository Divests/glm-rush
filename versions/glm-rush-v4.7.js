...
// @version      4.7
// @description  并发重试 + 自适应间隔 + 反检测 + check校验 + 弹窗恢复 + 定时触发 + 配置持久化 + 验证码处理
// @author       Assistant
...
(function () {
    ...
    async function singleAttempt(url, opts, attemptNum) {
        try {
            ...
            const resp = await _fetch(url, { ...opts, body: reqBody, headers: randHeaders, credentials: 'include' });
    ...
    async function retry(url, rawOpts) {
        ...
        _retryLock = (async () => {
            ...
            while (totalAttempt < CFG.maxRetry && !stopRequested) {
                ...
                // 验证码检测 — 100% 需要验证码时阻塞弹窗
                if (reasons.every(r => r === '需要验证码')) {
                    log('所有请求需要验证码, 弹出验证码...');
                    const captchaResult = await doCaptcha();
                    if (captchaResult.ticket) {
                        log(`验证码完成: ticket=${captchaResult.ticket.slice(0, 20)}...`);
                        try {
                            const bodyObj = _parse(opts.body || '{}');
                            opts.body = JSON.stringify({ ...bodyObj, ticket: captchaResult.ticket, randstr: captchaResult.randstr });
                        } catch {}
                    }
                    continue;
                }
...
<div class="panel">
  <div class="hd" id="drag"><b>GLM v4.7</b><button class="mn" id="min">-</button></div>
  <div class="bd" id="bd">
    ...
    <div class="keys">Alt+S 抢购 | Alt+X 停止 | Alt+H 隐藏</div>
        ...
        log('v4.7 已加载 (验证码处理+ticket清理+check容错)');
