...
// @version      4.8
// @description  并发重试 + 自适应间隔 + 反检测 + check校验 + 弹窗恢复 + 定时触发 + 配置持久化 + 验证码队列
// @author       Assistant
    ...
    async function retry(url, rawOpts) {
        ...
        _retryLock = (async () => {
            ...
            while (totalAttempt < CFG.maxRetry && !stopRequested) {
                ...
                // 验证码检测 — 50% 需要验证码时后台弹窗（非阻塞）
                const captchaCount = reasons.filter(r => r === '需要验证码').length;
                if (captchaCount > batchSize / 2) {
                    log(`${captchaCount}/${batchSize} 需要验证码, 后台弹窗...`);
                    backgroundSolveCaptcha();
                }
...
<div class="panel">
  <div class="hd" id="drag"><b>GLM v4.8</b><button class="mn" id="min">-</button></div>
  <div class="bd" id="bd">
    ...
    <div class="keys">Alt+S 抢购 | Alt+X 停止 | Alt+H 隐藏</div>
        ...
        log('v4.8 已加载 (验证码队列+后台预热+非阻塞50%阈值)');
