...
(function () {
    ...
    async function retry(url, rawOpts) {
        ...
        stopRequested = false;
        const opts = rawOpts || {};
        const abortCtrl = new AbortController(); // 成功后 abort 其余请求，释放连接池
        ...
        _retryLock = (async () => {
            ...
            const enqueue = () => {
                ...
                setTimeout(() => {
                    ...
                    singleAttempt(url, { ...opts, signal: abortCtrl.signal }, totalAttempt).then(result => {
                        inFlight--;
                        if (settled) return;

                        if (result.ok) {
                            // 成功！abort 其余在飞请求，释放连接池
                            settled = true;
                            winnerResult = result;
                            abortCtrl.abort();
                        } else {
            ...
            if (winnerResult) {
                ...
                stopCaptchaPreheat();
                broadcastSuccess(winnerResult.bizId); // 通知其他标签页停止
                recoveryAttempts = 0;
    ...
    let measuredRTT = 50;     // 实测 RTT 中位数(ms)
    ...
    async function syncServerTime() {
        ...
        for (let i = 0; i < 5; i++) {
            try {
                const t0 = Date.now();
                const resp = await _fetch(location.origin + `/api/biz/pay/check?bizId=sync_${i}`, { credentials: 'include' }).catch(() => null);
                const t1 = Date.now();
                const rtt = t1 - t0;

                if (resp && resp.headers.get('date')) {
                    const serverTime = new Date(resp.headers.get('date')).getTime();
                    offsets.push(serverTime - (t0 + rtt / 2));
                    rtts.push(rtt);
                }
            } catch {}
        ...
        if (offsets.length > 0) {
            ...
            measuredRTT = rtts[Math.floor(rtts.length / 2)];
            const localNow = new Date(Date.now() + serverTimeOffset);
            log(`时间同步: 偏差 ${serverTimeOffset > 0 ? '+' : ''}${serverTimeOffset}ms RTT=${measuredRTT}ms (${offsets.length}次采样)`);
    ...
    // ═══════════════════════════════════════════
    //  多标签页协同 (BroadcastChannel)
    // ═══════════════════════════════════════════
    const tabId = Math.random().toString(36).slice(2, 10);
    let bc = null;
    try {
        bc = new BroadcastChannel('glm-rush');
        bc.onmessage = (e) => {
            const msg = e.data;
            if (msg.type === 'success' && msg.tabId !== tabId) {
                log(`标签页 [${msg.tabId.slice(0,4)}] 已抢到 bizId=${msg.bizId}, 停止本标签页`);
                stopRequested = true;
                stopCaptchaPreheat();
                setState({ status: 'success', bizId: msg.bizId });
            }
        };
    } catch {} // BroadcastChannel 不可用时静默降级

    function broadcastSuccess(bizId) {
        if (bc) {
            try { bc.postMessage({ type: 'success', tabId, bizId }); } catch {}
        }
    }
    ...
    function scheduleAt(timeStr) {
        ...
        const rttAdvance = Math.max(Math.round(measuredRTT / 2), 50); // 使用实测 RTT
